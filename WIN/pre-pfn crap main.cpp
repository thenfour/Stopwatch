...
void SetDlgItemTime(HWND hwnd, int secsTotal)
{
  ...
  // x x : x x . x x 0
  WORD digits[128];
  char* pDigits = (char*)digits;
  DWORD* dwDigits = (DWORD*)digits;
...
lol:
    ...
    mov edi, sz0;// pointer into the string.
    mov esi, dwDigits;
    ...
    stosw;
    mov dword ptr [edi], 0x3a00002e;// 2e = ':', 3a = '.'
    inc edi;
  ...
  if(StringDoesNotEqual(sz, sz2))
  {
    ...
    {
      x_SetWindowTextA(g_CreateControlHParent, sz);
    }
  }
}
...
{
  x_SendMessageA(t.idProgress, PBM_SETBARCOLOR, 0, PBBarColors[i]);
  x_SendMessageA(t.idProgress, PBM_SETBKCOLOR, 0, PBBackColors[i]);
}
...
void UpdateTimer(Timer& p)
{
    ...
    else
    {
        ...
        {
          x_SendMessageA(p.idProgress, PBM_SETPOS, remainder, 0);
        }
    ...
    case WM_CREATE:
        {
          ...
          __CreateControl(BS_AUTOCHECKBOX, MakeRectAndID(0, IDC_ALWAYSONTOP, 0, 0, 1, 0, g_TimerCount), "BUTTON", "Always on top");
          ...
          int i = 0;
          Timer* pTimersEnd = pTimers + g_TimerCount;
          for(Timer* pTimer = pTimers; pTimer != pTimersEnd; pTimer ++)
          {
              pTimer->idMainDisplay = __CreateControl(ES_READONLY, MakeRectAndID(1, IDC_TIME + i, 0, 0, 2, 2, i), "EDIT", "");
              if(!g_firstTimerWindow)
              {
                g_firstTimerWindow = pTimer->idMainDisplay;
              }
              pTimer->idRealTotal  = __CreateControl(ES_READONLY, MakeRectAndID(1, IDC_TRUETOTAL + i, 3, 1, 0, 1, i), "EDIT", "");
              pTimer->idTotal = __CreateControl(0, MakeRectAndID(1, IDC_TOTAL + i, 4, 1, 0, 1, i), "EDIT", "");
              pTimer->idEnterTime = __CreateControl(0, MakeRectAndID(1, IDC_ENTERTIME + i, 1, 1, 0, 1, i), "EDIT", "");

              pTimer->idProgress = __CreateControl(1, MakeRectAndID(1, IDC_PROGRESS + i, 0, 2, 3, 3, i), "msctls_progress32", "");
              pTimer->idOnOffIndicator = __CreateControl(0, MakeRectAndID(0, IDC_LIST + i, 0, 3, 4, 1, i), "SysListView32", "");
              pTimer->idPauseButton = __CreateControl(0, MakeRectAndID(0, IDC_PAUSE + i, 3, 3, 0, 1, i), "BUTTON", "Pause");

              __CreateControl(0, MakeRectAndID(0, IDC_RESET + i, 4, 3, 0, 1, i), "BUTTON", "Reset");
              __CreateControl(0, MakeRectAndID(0, IDC_SETTIME + i, 2, 1, 0, 1, i), "BUTTON", "<-- Set Time");
              __CreateControl(0, MakeRectAndID(0, 0, 1, 0, 0, 3, i), "STATIC", "Enter Time");
              __CreateControl(0, MakeRectAndID(0, 0, 3, 0, 0, 3, i), "STATIC", "Destination Time");

              pTimer->idTimeLeft = __CreateControl(0, MakeRectAndID(0, IDC_TIMELEFT, 4, 0, 0, 3, i), "STATIC", "");

              x_SendMessageA(pTimer->idProgress, PBM_SETRANGE32, 0, g_DefaultTotal[i]);
              SetDlgItemTime(pTimer->idRealTotal, g_DefaultTotal[i]);
              x_SendMessageA(pTimer->idMainDisplay, WM_SETFONT, (WPARAM)hFont, TRUE);
              x_SendMessageA(pTimer->idOnOffIndicator, LVM_SETBKCOLOR, 0, Playing);
              UpdateTimer(*pTimer);
              i ++;
          }
...
WNDCLASS wcx = { CS_VREDRAW | CS_HREDRAW, WndProc, 0, 0, (HINSTANCE)0x00400000, 0, (HCURSOR)0x10013, (HBRUSH)(COLOR_BTNFACE + 1), 0, "]"};
...
int WINAPI WinMain(HINSTANCE ____, HINSTANCE _, PSTR __, int ___)
{
    ...
    x_CreateWindowExA(0, "]", "", (WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_VISIBLE) -WS_MAXIMIZEBOX, 0, 0, g_windowWidth, g_initialWindowHeight, 0, 0, (HINSTANCE)0, 0);
